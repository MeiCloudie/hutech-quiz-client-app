...
const requests = {
  get: <T>(url: string, params?: PaginationParams) =>
    axios
      .get<T>(baseUrl + url, { params: params?.toUrlSearchParams() })
      .then(responseBody),
  getFile: <T>(url: string, rt: ResponseType, params?: PaginationParams) =>
    axios
      .get<T>(baseUrl + url, { params: params?.toUrlSearchParams(), responseType: rt })
      .then(responseBody),
  post: <T>(url: string, body: {}) =>
    axios.post<T>(baseUrl + url, body).then(responseBody),
  put: <T>(url: string, body: {}) => axios.put<T>(baseUrl + url, body).then(responseBody),
  ...
  uploadFile: <T>(url: string, files: { name: string; blob: Blob }[]) => {
    ...
    return axios.post<T>(baseUrl + url, data, {
  ...
  > = {
    ...
    exportMultipleNonEntity: (firstEntityId: TFirstId) =>
      requests.getFile<Blob>(
        `v1/${firstEntityName}/${firstEntityId}/${secondEntityName}/export`,
        "blob"
      ),
  }
  return resource
