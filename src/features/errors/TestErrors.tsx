...
const TestErrors = () => {
  ...
  function handleNotFound() {
    agent.Results.notFound().catch((err) => console.log(err.response))
  }

  function handleBadRequest() {
    agent.Results.badRequest().catch((err) => console.log(err.response))
  }

  function handleNoContent() {
    agent.Results.noContent().catch((err) => console.log(err.response))
  }

  function handleUnauthorized() {
    agent.Results.unauthorized().catch((err) => console.log(err.response))
  }

  function handleConflict() {
    agent.Results.conflict().catch((err) => console.log(err.response))
  }

  function handleForbid() {
    agent.Results.forbid().catch((err) => console.log(err.response))
  }

  function handleInternalServerError() {
    agent.Results.internalServerError().catch((err) =>
